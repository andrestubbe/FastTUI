...
public class Window extends Component {
    ...
    public void render(FastTerminalScene scene) {
        ...
        for (int c = 0; c < currentTitleWidth; c++) {
            scene.writeCell(x + c, y, ' ', titleFgColor, titleBgColor);
        }

        // Draw title
        scene.writeString(x + 1, y, title, titleFgColor, titleBgColor);

        // Collapse/Expand button
        String toggleBtn = isExpanded ? "::" : "[]";
        scene.writeString(x + currentTitleWidth - 3, y, toggleBtn, titleFgColor, titleBgColor);
