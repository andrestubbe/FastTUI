...
- Uses `native\fasttui.def` for JNI exports
- Outputs to `build\fasttui.dll`

The Maven `pom.xml` will automatically pick up `build\fasttui.dll` and bundle it inside the JAR.

## JNI Exports (.def File)

When using JNI, you MUST export your native functions in the `native\fasttui.def` file:

```def
LIBRARY fasttui
EXPORTS
    Java_fasttui_FastTUI_doSomethingNative
```
